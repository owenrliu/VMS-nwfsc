...
The two CPUE measures are correlated by not extremely precisely related. Hence, care should be taken when a CPUE measure like this is used.
...
fishtix %>% 
  select(drvid,FINAL_LENGTH) %>% 
  distinct() %>% 
  ggplot(aes(FINAL_LENGTH))+
  geom_histogram(fill="blue",alpha=0.5,bins=25)+
...
An interesting thing to notice here is how the proportion of the fleet comprised of large boats is smallest at the beginning of each crab season (November and December). This might mean that a lot of small boats participate in the fishery for just a couple of months before dropping out, while the large boats are more consistent participants in the fishery.
