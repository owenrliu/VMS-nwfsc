...
mean(mean.lbs$percent.wVMS) #0.1997645
median(mean.lbs$percent.wVMS) #0.1983523

mean.rev <-sumdat.all.mean %>%
  dplyr::select(-dcrb.lbs) %>%
  spread(value=dcrb.rev,key=VMS) %>%
  summarise(percent.wVMS=Yes/(No+Yes))
mean(mean.rev$percent.wVMS) #0.1836726
median(mean.rev$percent.wVMS) #0.1798159
...
n.vessels.vms.all <- n.vessels.vms.all %>%
  spread(key=VMS,value=Freq) %>%
  mutate(percent.wVMS = Yes/(Yes+No))
mean(n.vessels.vms.all$percent.wVMS) #0.1233715
median(n.vessels.vms.all$percent.wVMS) #0.1169916
...
mean.lbs <-sumdat.all.mean %>%
  ...
  spread(value=dcrb.lbs,key=VMS) %>%
  summarise(percent.wVMS=Yes/(No+Yes))
mean((mean.lbs %>% filter(vessel_cat=="large"))$percent.wVMS)
mean((mean.lbs %>% filter(vessel_cat=="small"))$percent.wVMS)
median((mean.lbs %>% filter(vessel_cat=="large"))$percent.wVMS) #0.2453443
median((mean.lbs %>% filter(vessel_cat=="small"))$percent.wVMS) #0.03943656
...
mean.rev <-sumdat.all.mean %>%
  ...
  spread(value=dcrb.rev,key=VMS) %>%
  summarise(percent.wVMS=Yes/(No+Yes))
mean((mean.rev %>% filter(vessel_cat=="large"))$percent.wVMS)
mean((mean.rev %>% filter(vessel_cat=="small"))$percent.wVMS)
median((mean.rev %>% filter(vessel_cat=="large"))$percent.wVMS) #0.2394658
median((mean.rev %>% filter(vessel_cat=="small"))$percent.wVMS) #0.03160455
...
mean(ncrab_vessels$nvessels) # 340.8571
...
# join tickets, lengths, and vms
dcrb_tix <- left_join(dcrb_tix, vlengths,by=c("drvid","agency_code","crab_year"))
dcrb_tix <- left_join(dcrb_tix, nvms, by=c("drvid"="DOCNUM", "Rec_ID"))
dcrb_tix <- dplyr::select(dcrb_tix, Rec_ID, port_group_code, drvid, agency_code, removal_type_code, date, month, year, crab_year, region, mean_length, vessel_cat, n.dcrb.records)
dcrb_tix$n.dcrb.records[which(is.na(dcrb_tix$n.dcrb.records))] <- 0



dcrb_vessels <- dcrb_tix %>%
  mutate(hasVMS = ifelse(n.dcrb.records > 0, "yes", "no")) %>%
  group_by(drvid,vessel_cat,crab_year) %>%
  summarise(n.dcrb.trips = length(unique(Rec_ID)), n.dcrb.trips.vms = sum(hasVMS=="yes"), n.dcrb.records=sum(n.dcrb.records))

dcrb_vessels_orig <- filter(matchdat, crab_year==2010)

compare_methods <- left_join(dcrb_vessels_orig,dcrb_vessels, by=c("drvid","vessel_cat","crab_year"))
compare_methods <- compare_methods %>%
  dplyr::select(-crab_year, -mean_length, -agency_code, -VMS) %>%
  dplyr::select(drvid, vessel_cat, n.dcrb.trips.x,n.dcrb.trips.y,n.dcrb.trips.vms.x,n.dcrb.trips.vms.y,n.dcrb.records.x,n.dcrb.records.y)
compare_methods_diff <- compare_methods %>%
  mutate(diff.dcrb.trips=n.dcrb.trips.x-n.dcrb.trips.y,
         diff.vms.trips=n.dcrb.trips.vms.x-n.dcrb.trips.vms.y,
         diff.vms.records=n.dcrb.records.x-n.dcrb.records.y) %>%
  dplyr::select(drvid,vessel_cat,diff.dcrb.trips,diff.vms.trips,diff.vms.records)
sum(compare_methods_diff$diff.dcrb.trips > 0)
sum(compare_methods_diff$diff.vms.trips > 0)
sum(compare_methods_diff$diff.vms.records > 0)
...
################ testing 123...2015 --------------------------------------------------------------------
tmpvms1 <- read.csv(paste0("R_Output/match/unfiltered/VMS_Outputs_wTARGET_10d_lookback_2015_all.csv"))
tmpvms2 <- read.csv(paste0("R_Output/match/unfiltered/VMS_Outputs_wTARGET_10d_lookback_2016_all.csv"))
tmpvms2 <- tmpvms2[,colnames(tmpvms1)]
tmpvms <- rbind(tmpvms1,tmpvms2);rm(tmpvms1,tmpvms2)
vms_nodup <- tmpvms[c(which(!(duplicated(subset(tmpvms,select=c(DOCNUM, UTCDATETIM)),fromLast=TRUE)))),]
## counts
...
dcrb_tix <- tickets_filtered %>%
  ...
  filter(TARGET=="DCRB")
vlengths <- filter(lkey_filtered, crab_year == 2015)
...
dcrb_vessels_orig <- filter(matchdat, crab_year==2015)
...
dcrb_tix <- left_join(dcrb_tix, vlengths,by=c("drvid","agency_code","crab_year"))
dcrb_tix <- left_join(dcrb_tix, nvms, by=c("drvid"="DOCNUM", "Rec_ID"))
dcrb_tix <- dplyr::select(dcrb_tix, Rec_ID, port_group_code, drvid, date, month, crab_year, region, mean_length, vessel_cat, DCRB..lbs.,DCRB...., n.dcrb.records)
dcrb_tix$n.dcrb.records[which(is.na(dcrb_tix$n.dcrb.records))] <- 0
dcrb_tix <- dcrb_tix %>% mutate(VMS = ifelse(n.dcrb.records > 0, "Yes", "No"))

dcrb_trips <- dcrb_tix %>%
  group_by(drvid,crab_year,vessel_cat,month,VMS) %>%
  summarise(total.lbs=sum(DCRB..lbs.),total.rev=sum(DCRB....))
dcrb_trips_orig <- filter(matchdat.trips, crab_year==2015)
compare_methods <- left_join(dcrb_trips_orig,dcrb_trips, by=c("drvid","crab_year","month","VMS"))

compare_methods_diff <- compare_methods %>%
  dplyr::select(-crab_year, -mean_length) %>%
  dplyr::select(drvid, vessel_cat, VMS, total.lbs.x,total.lbs.y,total.rev.x,total.rev.y) %>%
  mutate(diff.rev=total.lbs.x-total.lbs.y,
         diff.lbs=total.rev.x-total.rev.y) %>%
  dplyr::select(drvid,vessel_cat,VMS, diff.rev,diff.lbs)
sum(compare_methods_diff$diff.rev > 0)
sum(compare_methods_diff$diff.lbs > 0)
