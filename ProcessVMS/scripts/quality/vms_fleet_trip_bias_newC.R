...
for(y in years[2:length(years)]){
  ...
  sumdat <- tmpdat %>%
    ...
    summarise(total.lbs=sum(DCRB..lbs.),total.rev=sum(DCRB....), total.trips=length(unique(Rec_ID)))
  if(y==years[2]){
...
sumdat <- matchdat.trips %>% 
  ...
  summarise(dcrb.lbs=sum(total.lbs),dcrb.rev=sum(total.rev), dcrb.trips=sum(total.trips)) %>%
  filter(dcrb.trips > 3)
...
#trips
myplot <- ggplot(data=filter(sumdat,!is.na(vessel_cat)),aes(x=month,y=dcrb.trips,fill=VMS)) +
  geom_col() +
  facet_grid(cols=vars(crab_year),rows=vars(vessel_cat),scales="free_y") +
  xlab("Month") + ylab("D. crab Trips") +
  ggtitle(paste0(state, " Dungeness crab Vessels\n Matched VMS")) +
  scale_fill_manual(values=c("coral2","cyan3")) +
  ylim(c(0,2000)) +
  scale_x_discrete(name="Month",labels=c("","Dec","","Feb","","Apr","","Jun","","Aug","","Oct")) +
  theme_bw() + theme(panel.grid.major.y=element_blank(),panel.grid.minor.y=element_blank(),
                     ...
                     axis.title=element_text(size=14),title = element_text(size=15),
                     legend.text=element_text(size=12),legend.title=element_text(size=14))
png(paste0("R_Output/quality/vms_coverage/dcrb_trips_wVMS_bycat_matchVMS_yearly_",states[1],".png"),width=720,height=400)
myplot
dev.off()

myplot <- ggplot(data=sumdat,aes(x=month,y=dcrb.trips,fill=VMS)) +
  geom_col() +
  facet_grid(cols=vars(crab_year),rows=vars(vessel_cat),scales="free_y") +
  xlab("Month") + ylab("D. crab Trips") +
  ggtitle(paste0(state, " Dungeness crab Vessels\n Matched VMS")) +
  ...
  theme_bw() + theme(panel.grid.major.y=element_blank(),panel.grid.minor.y=element_blank(),
                     ...
                     axis.title=element_text(size=14),title = element_text(size=15),
                     legend.text=element_text(size=12),legend.title=element_text(size=14))
png(paste0("R_Output/quality/vms_coverage/dcrb_trips_wVMS_bycat_matchVMS_yearly_",states[1],"_wNA.png"),width=720,height=400)
myplot
dev.off()
...
sumdat.trips <- sumdat %>%
  dplyr::select(-dcrb.rev, -dcrb.lbs) %>%
  spread(key=VMS,value=dcrb.trips)
sumdat.trips$No[which(is.na(sumdat.trips$No))] <- 0
sumdat.trips$Yes[which(is.na(sumdat.trips$Yes))] <- 0
sumdat.trips <- sumdat.trips %>%
  mutate(`without VMS` = No / (No + Yes), `with VMS` = Yes / (No + Yes)) %>%
  dplyr::select(-No,-Yes) %>%
  melt(id.vars=c("crab_year","month","vessel_cat"))
sumdat.trips$month <- factor(sumdat.trips$month, levels=c("Nov","Dec","Jan","Feb",
                                                      "Mar","Apr","May","Jun","Jul","Aug",
                                                      "Sep","Oct"))

#trips
myplot <- ggplot(data=filter(sumdat.trips, !is.na(vessel_cat)),aes(x=factor(month),y=value,fill=variable)) +
  geom_col(width=1) +
  facet_grid(cols=vars(crab_year),rows=vars(vessel_cat),scales="free_y") +
  ylab("Proportion D. crab Fishing Trips") +
  ggtitle(paste0(state, " Dungeness crab Vessels\n Matched VMS")) +
  scale_fill_manual(values=c("coral2","cyan3")) +
  scale_x_discrete(name="Month",labels=c("","Dec","","Feb","","Apr","","Jun","","Aug","","Oct")) +
  theme_bw() + theme(panel.grid.major=element_blank(),panel.grid.minor=element_blank(),
                     ...
                     legend.text=element_text(size=12),legend.title=element_blank(),
                     strip.text=element_text(size=12))
png(paste0("R_Output/quality/vms_coverage/dcrb_proptrips_wVMS_bycat_matchVMS_yearly_",states[1],".png"),width=820,height=400)
myplot
dev.off()
...
#trips
sumdat.all.trips <- sumdat.all %>%
  dplyr::select(-dcrb.lbs,-dcrb.rev) %>%
  spread(key=VMS,value=dcrb.trips)
sumdat.all.trips$No[which(is.na(sumdat.all.trips$No))] <- 0
sumdat.all.trips$Yes[which(is.na(sumdat.all.trips$Yes))] <- 0
sumdat.all.trips <- sumdat.all.trips %>%
  mutate(`without VMS` = No / (No + Yes), `with VMS` = Yes / (No + Yes)) %>%
  dplyr::select(-No,-Yes) %>%
  melt(id.vars=c("crab_year","month"))


myplot <- ggplot(data=sumdat.all.trips,aes(x=month,y=value,fill=variable)) +
  geom_col(width=1) +
  facet_wrap(~crab_year,ncol=4) +
  xlab("Month") + ylab("Proportion of D. Crab Fishing Trips") +
  ggtitle(paste0(state, " Dungeness crab Vessels\n Matched VMS")) +
  scale_fill_manual(values=c("coral2","cyan3")) +
  theme_bw() + theme(panel.grid.major=element_blank(),panel.grid.minor=element_blank(),
                     axis.text.x=element_text(angle=90,hjust=1,vjust=0,size=12),
                     ...
                     legend.text=element_text(size=12),legend.title=element_blank(),
                     strip.text=element_text(size=12))
png(paste0("R_Output/quality/vms_coverage/dcrb_proptrips_wVMS_matchVMS_yearly_",states[1],".png"),width=820,height=400)
myplot
dev.off()
...
mean(mean.lbs$percent.wVMS) #0.3398578
median(mean.lbs$percent.wVMS) #0.3432644

mean.rev <-sumdat.all.mean %>%
  dplyr::select(-dcrb.lbs) %>%
  spread(value=dcrb.rev,key=VMS) %>%
  summarise(percent.wVMS=Yes/(No+Yes))
mean(mean.rev$percent.wVMS) #0.3346352
median(mean.rev$percent.wVMS) #0.3379119
...
n.vessels.vms.all <- n.vessels.vms.all %>%
  spread(key=VMS,value=Freq) %>%
  mutate(percent.wVMS = Yes/(Yes+No))
mean(n.vessels.vms.all$percent.wVMS) #0.2769243
median(n.vessels.vms.all$percent.wVMS) #0.2914894
...
mean.lbs <-sumdat.all.mean %>%
  ...
  spread(value=dcrb.lbs,key=VMS) %>%
  summarise(percent.wVMS=Yes/(No+Yes))
mean((mean.lbs %>% filter(vessel_cat=="large"))$percent.wVMS) #0.3934576
mean((mean.lbs %>% filter(vessel_cat=="small"))$percent.wVMS) #0.2169438
median((mean.lbs %>% filter(vessel_cat=="large"))$percent.wVMS) #0.3793867
median((mean.lbs %>% filter(vessel_cat=="small"))$percent.wVMS) #0.2145883
...
mean.rev <-sumdat.all.mean %>%
  ...
  spread(value=dcrb.rev,key=VMS) %>%
  summarise(percent.wVMS=Yes/(No+Yes))
mean((mean.rev %>% filter(vessel_cat=="large"))$percent.wVMS) #0.3902789
mean((mean.rev %>% filter(vessel_cat=="small"))$percent.wVMS) #0.2121554
median((mean.rev %>% filter(vessel_cat=="large"))$percent.wVMS) #0.3723807
median((mean.rev %>% filter(vessel_cat=="small"))$percent.wVMS) #0.2136575
...
mean(ncrab_vessels$nvessels) #449.2857
